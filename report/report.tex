...
Table~\ref{tab:path_repr_crossover} contains the results of experiments with different crossover operators. The function 'cross\_alternating\_edges' implements Alternating Edge Crossover and is provided in the template. Order Crossover performs significantly better than Alternating Edge Crossover; all of the path lengths with Order Crossover are lower for every dataset. Also, the path lengths converted to a value earlier with Order Crossover, which is why the numbers of generations are lower. This can be attributed to \textbf{... ???}
...
\label{sec:local}
For this task, we are testing the local optimization already present in the template. This optimization takes a path and tries to remove local loops with a path length of up to 3. With default values for other parameters, this results in major improvements to the score.
